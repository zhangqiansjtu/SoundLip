...
def dcnn(Iinputs,Qinputs, train,keep_prob, reuse=False, name='cnnBN'):
    with tf.variable_scope(name, reuse=reuse) as scope:


        Iconv1 = tf.layers.conv2d(Iinputs, 8, (8, 3), dilation_rate=(1,1), padding='same')
        IBN_out1 = bn_layer(Iconv1,train,scope='IBN1')
        Ih_conv1 = tf.nn.relu(IBN_out1)
        Ih_pool1 = tf.nn.max_pool(Ih_conv1, ksize=[1, 2, 1, 1], strides=[1, 2, 1, 1], padding='SAME')


        Qconv1 = tf.layers.conv2d(Qinputs, 8, (8, 3), dilation_rate=(1,1), padding='same')
        QBN_out1 = bn_layer(Qconv1,train,scope='QBN1')
        Qh_conv1 = tf.nn.relu(QBN_out1)
        Qh_pool1 = tf.nn.max_pool(Qh_conv1, ksize=[1, 2, 1, 1], strides=[1, 2, 1, 1], padding='SAME')

## 第二层卷积操作 ##
        Iconv2 = tf.layers.conv2d(Ih_pool1, 16, (8, 3), dilation_rate=(1,1), padding='same')
        IBN_out2 = bn_layer(Iconv2,train,scope='IBN2')
        Ih_conv2 = tf.nn.relu(IBN_out2)
        Ih_pool2 = tf.nn.max_pool(Ih_conv2, ksize=[1, 2, 1, 1], strides=[1, 2, 1, 1], padding='SAME')

        Qconv2 = tf.layers.conv2d(Qh_pool1, 16, (8, 3), dilation_rate=(1,1), padding='same')
        QBN_out2 = bn_layer(Qconv2,train,scope='QBN2')
        Qh_conv2 = tf.nn.relu(QBN_out2)
        Qh_pool2 = tf.nn.max_pool(Qh_conv2, ksize=[1, 2, 1, 1], strides=[1, 2, 1, 1], padding='SAME')

        Iconv3 = tf.layers.conv2d(Ih_pool2, 32, (8, 3), dilation_rate=(1,1), padding='same')
        IBN_out3 = bn_layer(Iconv3, train, scope='IBN3')
        Ih_conv3 = tf.nn.relu(IBN_out3)
        Ih_pool3 = tf.nn.max_pool(Ih_conv3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

        Qconv3 = tf.layers.conv2d(Qh_pool2, 32, (8, 3), dilation_rate=(1,1), padding='same')
        QBN_out3 = bn_layer(Qconv3, train, scope='QBN3')
        Qh_conv3 = tf.nn.relu(QBN_out3)
        Qh_pool3 = tf.nn.max_pool(Qh_conv3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

        Iconv4 = tf.layers.conv2d(Ih_pool3, 64, (8, 3), dilation_rate=(1,1), padding='same')
        IBN_out4 = bn_layer(Iconv4, train, scope='IBN4')
        Ih_conv4 = tf.nn.relu(IBN_out4)
        Ih_pool4 = tf.nn.max_pool(Ih_conv4, ksize=[1, 3, 1, 1], strides=[1, 3, 1, 1], padding='SAME')

        Qconv4 = tf.layers.conv2d(Qh_pool3, 64, (8, 3), dilation_rate=(1,1), padding='same')
        QBN_out4 = bn_layer(Qconv4, train, scope='QBN4')
        Qh_conv4 = tf.nn.relu(QBN_out4)
        Qh_pool4 = tf.nn.max_pool(Qh_conv4, ksize=[1, 3, 1, 1], strides=[1, 3, 1, 1], padding='SAME')

        Iconv5 = tf.layers.conv2d(Ih_pool4, 128, (5, 3), dilation_rate=(1,1), padding='same')
        IBN_out5 = bn_layer(Iconv5, train, scope='IBN5')
        Ih_conv5 = tf.nn.relu(IBN_out5)
        Ih_pool5 = tf.nn.max_pool(Ih_conv5, ksize=[1, 3, 2, 1], strides=[1, 3, 2, 1], padding='SAME')

        Qconv5 = tf.layers.conv2d(Qh_pool4, 128, (5, 3), dilation_rate=(1,1), padding='same')
        QBN_out5 = bn_layer(Qconv5, train, scope='QBN5')
        Qh_conv5 = tf.nn.relu(QBN_out5)
        Qh_pool5 = tf.nn.max_pool(Qh_conv5, ksize=[1, 3, 2, 1], strides=[1, 3, 2, 1], padding='SAME')

        Iconv6 = tf.layers.conv2d(Ih_pool5, 128, (5, 4), dilation_rate=(1, 1), padding='same')
        IBN_out6 = bn_layer(Iconv6, train, scope='IBN6')
        Ih_conv6 = tf.nn.relu(IBN_out6) + Ih_pool5

        Qconv6 = tf.layers.conv2d(Qh_pool5, 128, (5, 4), dilation_rate=(1, 1), padding='same')
        QBN_out6 = bn_layer(Qconv6, train, scope='QBN6')
        Qh_conv6 = tf.nn.relu(QBN_out6) + Qh_pool5



        Ih_conv = tf.expand_dims(Ih_conv6, 3)
        Qh_conv = tf.expand_dims(Qh_conv6, 3)
        sensor_conv = tf.concat([Ih_conv,Qh_conv], 3)
        sensorw_conv1 = weight_variable([5, 4, 2, 128, 128])
        sensorb_conv1 = bias_variable([128])
        sensorconv1 = tf.nn.conv3d(sensor_conv, sensorw_conv1, strides=[1, 1, 1, 1, 1], padding='SAME') + sensorb_conv1
        sensorBN_out1 = bn_layer(sensorconv1, train, scope='sensorBN1')
        sensor_conv1 = tf.nn.relu(sensorBN_out1)
        # sensor_conv1 = tf.nn.max_pool3d(sensor_conv1, ksize=[1, 3, 1, 1, 1], strides=[1, 3, 1, 1, 1], padding='SAME')
        sensorw_conv2 = weight_variable([3, 4, 2, 128, 32])
        sensorb_conv2 = bias_variable([32])
        sensorconv2 = tf.nn.conv3d(sensor_conv1, sensorw_conv2, strides=[1, 1, 1, 1, 1], padding='SAME') + sensorb_conv2
        sensorBN_out2 = bn_layer(sensorconv2, istraining, scope='sensorBN2')
        sensor_conv2 = tf.nn.relu(sensorBN_out2)
...
def cnn1D(Iinputs,Qinputs, train,keep_prob, reuse=False, name='cnnBN'):
    with tf.variable_scope(name, reuse=reuse) as scope:
        Iinputs = tf.reshape(Iinputs, [-1, Iinputs.shape[1], Iinputs.shape[2]])
        Iconv1 = tf.layers.conv1d(Iinputs, 32, 8, strides=1, padding='same', dilation_rate=1)
        IBN_out1 = bn_layer(Iconv1, train, scope='IBN1')
        Ih_conv1 = tf.nn.relu(IBN_out1)
        Ih_pool1 = tf.layers.max_pooling1d(Ih_conv1, 2, 2, padding='SAME')

        Iconv2 = tf.layers.conv1d(Ih_pool1, 64, 8, strides=1, padding='same', dilation_rate=2)
        IBN_out2 = bn_layer(Iconv2, train, scope='IBN2')
        Ih_conv2 = tf.nn.relu(IBN_out2)
        Ih_pool2 = tf.layers.max_pooling1d(Ih_conv2, 2, 2, padding='SAME')

        Iconv3 = tf.layers.conv1d(Ih_pool2, 128, 8, strides=1, padding='same', dilation_rate=1)
        IBN_out3 = bn_layer(Iconv3, train, scope='IBN3')
        Ih_conv3 = tf.nn.relu(IBN_out3)
        Ih_pool3 = tf.layers.max_pooling1d(Ih_conv3, 2, 2, padding='SAME')

        Iconv4 = tf.layers.conv1d(Ih_pool3, 128, 8, strides=1, padding='same', dilation_rate=2)
        IBN_out4 = bn_layer(Iconv4, train, scope='IBN4')
        Ih_conv4 = tf.nn.relu(IBN_out4)
        Ih_pool4 = tf.layers.max_pooling1d(Ih_conv4, 3, 3, padding='SAME')

        Iconv5 = tf.layers.conv1d(Ih_pool4, 128, 8, strides=1, padding='same', dilation_rate=1)
        IBN_out5 = bn_layer(Iconv5, train, scope='IBN5')
        Ih_conv5 = tf.nn.relu(IBN_out5)
        Ih_pool5 = tf.layers.max_pooling1d(Ih_conv5, 3, 3, padding='SAME')

        Iconv6 = tf.layers.conv1d(Ih_pool5, 128, 8, strides=1, padding='same', dilation_rate=1)
        IBN_out6 = bn_layer(Iconv6, train, scope='IBN6')
        Ih_conv6 = tf.nn.relu(IBN_out6 + Ih_pool5)

        Qinputs = tf.reshape(Qinputs, [-1, Qinputs.shape[1], Qinputs.shape[2]])
        Qconv1 = tf.layers.conv1d(Qinputs, 32, 8, strides=1, padding='same', dilation_rate=1)
        QBN_out1 = bn_layer(Qconv1, train, scope='QBN1')
        Qh_conv1 = tf.nn.relu(QBN_out1)
        Qh_pool1 = tf.layers.max_pooling1d(Qh_conv1, 2, 2, padding='SAME')

        Qconv2 = tf.layers.conv1d(Qh_pool1, 64, 8, strides=1, padding='same', dilation_rate=2)
        QBN_out2 = bn_layer(Qconv2, train, scope='QBN2')
        Qh_conv2 = tf.nn.relu(QBN_out2)
        Qh_pool2 = tf.layers.max_pooling1d(Qh_conv2, 2, 2, padding='SAME')

        Qconv3 = tf.layers.conv1d(Qh_pool2, 128, 8, strides=1, padding='same', dilation_rate=1)
        QBN_out3 = bn_layer(Qconv3, train, scope='QBN3')
        Qh_conv3 = tf.nn.relu(QBN_out3)
        Qh_pool3 = tf.layers.max_pooling1d(Qh_conv3, 2, 2, padding='SAME')

        Qconv4 = tf.layers.conv1d(Qh_pool3, 128, 8, strides=1, padding='same', dilation_rate=2)
        QBN_out4 = bn_layer(Qconv4, train, scope='QBN4')
        Qh_conv4 = tf.nn.relu(QBN_out4)
        Qh_pool4 = tf.layers.max_pooling1d(Qh_conv4, 3, 3, padding='SAME')

        Qconv5 = tf.layers.conv1d(Qh_pool4, 128, 8, strides=1, padding='same', dilation_rate=1)
        QBN_out5 = bn_layer(Qconv5, train, scope='QBN5')
        Qh_conv5 = tf.nn.relu(QBN_out5)
        Qh_pool5 = tf.layers.max_pooling1d(Qh_conv5, 3, 3, padding='SAME')

        Qconv6 = tf.layers.conv1d(Qh_pool5, 128, 8, strides=1, padding='same', dilation_rate=1)
        QBN_out6 = bn_layer(Qconv6, train, scope='QBN6')
        Qh_conv6 = tf.nn.relu(QBN_out6 + Qh_pool5)



        h_c1 = tf.concat([Ih_conv6,Qh_conv6],2)

        conv1 = tf.layers.conv1d(h_c1, 256, 3, strides=1, padding='same', dilation_rate=1)
        BN_out1 = bn_layer(conv1, train, scope='BN1')
        h_conv1 = tf.nn.relu(BN_out1 + h_c1)

        conv2 = tf.layers.conv1d(h_conv1, 32, 3, strides=1, padding='same', dilation_rate=1)
        ...
        f_units1 = 128
        W_fc1 = weight_variable([h_conv_shape[1] * h_conv_shape[2], f_units1])
...
x_I = [x_image_split[0]]
x_Q = [x_image_split[1]]
x_I = tf.reshape(x_I, [-1, my_data.traindata.shape[1], 16, 1])
x_Q = tf.reshape(x_Q, [-1, my_data.traindata.shape[1], 16, 1])
logits,fc1 = cnn1D(x_I, x_Q, istraining, keep_prob, name='cnnBN')
